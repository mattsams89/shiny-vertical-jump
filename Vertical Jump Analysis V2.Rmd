...
renderPlotly({
  ...
  sampling_frequency <- input$sampling_frequency
  offset_data <- jump_information$offset_data
  offset_data[, time := 1:nrow(offset_data)][, time := time / sampling_frequency]
  
  plot_ly(offset_data, x = ~time) %>%
    add_lines(y = ~fp1,
              name = "FP1") %>%
    add_lines(y = ~fp2,
              name = "FP2") %>%
    ...
    layout(xaxis = list(title = "Time"),
           ...
           shapes = list(vline(jump_information$peak_force_index / sampling_frequency),
                         vline(jump_information$takeoff_index / sampling_frequency),
                         vline(jump_information$landing_index / sampling_frequency),
                         vline(jump_information$peak_landing_force_index / sampling_frequency),
                         vline(jump_information$jump_start_index / sampling_frequency)))
})
```
